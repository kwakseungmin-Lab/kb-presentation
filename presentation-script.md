# Knowledge Base 기초 - 발표 스크립트

## 슬라이드 0: 타이틀 (30초)

안녕하세요. Knowledge Base에 대해 그동안 습득한 내용들을 공유드리도록 하겠습니다.

KB가 뭔지, 어떻게 저장하고, 조회하는지 설계 전에 알아야 할 내용들을 정리해놓았습니다.
...
첫째, retrieve 동작 방식. 임베딩으로 뭘 찾고 릴레이션을 어떻게 따라가는지.

둘째, top-k랑 hops 차이. 각각 뭘 제어하는지.

셋째, 엔티티와 릴레이션 역할. 뭐가 저장되고 언제 쓰이는지.

넷째, KB를 쓰는 이유. 내 문제에서 왜 필요한지들을 알아봤고 azle UI에서의 검증을 통해 azle에서 말해준것과 실제 저장 조회 구조가 맞는지 검증하는 것들을 진행했었습니다.
...
릴레이션이 중요한 이유는 맥락 추적이 가능하고, hop expansion으로 주변 그래프를 펼칠 수 있고, 관계를 통해 결과를 추적할 수 있고, 방향과 predicate로 정확한 표현이 가능하기 때문입니다.

릴레이션이 KB를 단순 검색 인덱스가 아니라 구조적 기억으로 만들어주는 핵심이라고 봐주시면 될 것 같습니다.

---

## 슬라이드 6: 검색 vs 조회 (섹션) (15초)

검색과 조회의 차이입니다.

검색은 질의로 찾아내는 것, 조회는 이미 저장된 걸 읽는 것인데요
...
검색 첫 번째, EXACT/METADATA LOOKUP입니다. 정확한 ID나 key를 알 때 씁니다. 특정 entity_id 직접 조회나 label, properties 필터링용이고, 의미 유사도 검색이 아니라 정확한 매치입니다.

검색 두 번째, SEMANTIC RETRIEVAL입니다. retrieve_kb_graph인데요. embedding 기반 유사도 검색, top-k ranking으로 후보 선별, hop expansion으로 그래프 확장을 합니다. exact entity_id lookup은 보장이 안 되고, 개념이나 패턴 찾기에 적합합니다.

조회 4가지는 browse_kb_entities로 목록 훑기, snapshot으로 전체 상태 보기, get_kb_entity_neighbors로 주변 그래프 보기, get_kb_stats로 통계 보기가 있었습니다.
...
자동 INGEST는 불가능합니다. sync_repository_to_kb는 Markdown only입니다.

하지만 properties로는 가능한데요. upsert_kb_entity와 upsert_kb_relation이 properties_json을 받습니다.

JSON 처리는 Python 파싱 → 재귀 탐색 → root/object/array/value chunk → parent-child 관계 생성 순서입니다.

repo 전체 JSON 말고 schema, meta, config 성격만 선택적으로 넣는 걸 권장합니다. 노이즈 방지용이고요.

---

## 슬라이드 13: 왜 KB를 쓰는가 (1분 30초)

Art Task Plan iteration에 적합한가? 라는 관점에서 KB를 쓰는 이유를 보면,

KB를 쓰는 이유는 문서를 저장하기 위해서가 아니라, 반복 실행 사이에서 재사용해야 하는 관계, 상태, 이력을 구조적으로 기억시켜 다음 의사결정에 쓰기 위해서입니다.

문서는 설명과 공유에 강하고, 원천이나 보고서, 최종 근거가 목적이면 문서가 낫습니다. 

반면 KB는 구조화된 재사용 가능에 강하고, 다음 실행 현장에 다시 써야 하는 기억이면 KB가 낫고요.

다음 iteration 실행 방식을 결정할 때, 다음 iteration의 strategy, prompt, Evolver mode, verification criteria 같은 걸 정해야 하는데,

이걸 최근 iteration 결과, 실패 패턴, 통계 추세, 전략 이력을 보고 판단합니다.

본질은 과거 경과, 실패 패턴, 근거를 남기는 히스토리/메모리 계층이고, 조회 기능학고 추적 가능해야 합니다. KB는 최신 진실을 보장하는 장치가 아니라, 기억을 외부화하고 재조회 가능하게 만드는 장치에 가깝습니다.

---

## 슬라이드 14: 설계 원칙 (1분 30초)
